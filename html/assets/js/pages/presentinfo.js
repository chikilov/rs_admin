        ...
        }, function( isConfirm )
        {
	        if ( parseInt( mailmax[$('#item_id').val()] ) >= parseInt( $('#v1').val() ) )
	        {
				if (isConfirm) {
					$.ajax({
						method: "POST",
						url: "/Character/sendpresent",
						data: {'uid':$('#searchuid').val(), 'type':$('#item_id').val(), 'v1':$('#v1').val(), 'logc':$('#logc').val(), 'logt':$('#logt').val(), 'term': $('#term').val(), 'sendtext': $('#sendtext').val(), 'admin_memo':$('#admin_memo').val()},
						dataType: 'html',
